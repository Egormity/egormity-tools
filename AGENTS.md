...
- Keep tool code under focused package folders. The current tool package is `egormity_packages/egormity_git_tools`.
- Root-level scripts should be setup or orchestration scripts only.

## Development
- Keep command-line behavior stable and explicit.
- Add new tool commands through `egormity_packages/egormity_git_tools/cli.py` unless a new package is clearly warranted.
...
- Run `python -m compileall egormity_packages/egormity_git_tools` after Python code changes, then remove generated cache files.

## Versioning
- Update `egormity_packages/egormity_git_tools/version.py` when user-visible CLI behavior changes.
