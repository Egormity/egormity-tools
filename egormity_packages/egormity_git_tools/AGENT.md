...
- It is intended to be runnable from anywhere after `egormity_packages/set_windows_paths.ps1` has configured `PATH` and `PYTHONPATH`.

## Public Entry Points
- Direct command: `egormity_git_tools`
- Module command: `python -m egormity_git_tools`
- Windows shim: `egormity_packages/bin/egormity_git_tools.cmd`
